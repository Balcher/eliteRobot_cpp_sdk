...
/// @brief 从buffer中提取位姿信息
/// @param buffer 带有位姿信息的字符串
/// @return 位姿信息数组
std::vector<double> getposesMessage(std::string buffer)
{
    std::vector<double> numbers;
    std::regex regex(R"(-?\d+\.\d+|-?\d+)"); // 正则表达式匹配数字
...
bool Robot40011Port::getFKPose(const std::vector<double> &degrees, std::vector<double> &poses)
{
    // 1. 生成查询字符串
    std::string message = generateMessages("get_forward_kin", degrees);
    // std::cout << message << std::endl;
    ...
    // 5. 提取位置信息
    poses = getposesMessage(values[2]);
    return true;
}
//////////////////////////////////////////////////////////////////////////////////////////////////////
