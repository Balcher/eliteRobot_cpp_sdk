...
int main(int argc, char **argv)
{
    ...
    std::vector<float> jointTorque; // 力矩
    std::vector<double> tcpOffset;  // tcp偏移量
    std::vector<double> degrees;    // 关节角度
    while (true)
    {
        ...
        jointTorque = robot->getJointTorques();
        tcpOffset = robot->getTcpOffset();
        // 获取打印机器人关节位置
        for (auto item : joint)
        {
            printf("%.3lf\t", item);
        }
        // for (auto item : jointTorque)
        // {
        //     printf("%.3lf\t", item);
        // }
        // for (auto item : tcpOffset)
        // {
        //     printf("%.3lf\t", item);
        // }
